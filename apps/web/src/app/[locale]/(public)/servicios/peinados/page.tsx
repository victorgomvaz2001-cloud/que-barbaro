...
export default async function PeinadosPage() {
  const [locale, t] = await Promise.all([getLocale(), getTranslations('serviciosPage')])
  const service = t.raw('peinados') as ServiceData
  const seoRoute = locale === 'es' ? '/servicios/peinados' : `/${locale}/servicios/peinados`
